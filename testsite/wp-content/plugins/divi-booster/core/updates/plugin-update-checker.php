 ...
 * @access public
 */
class Divi_Booster_PluginUpdateChecker {
	...
	public function requestInfo($queryArgs = array()){
		...
		$queryArgs['installed_version'] = ($installedVersion !== null) ? urlencode($installedVersion) : '';
		...
		if (!empty($queryArgs)){ $url = add_query_arg($queryArgs, $url); }
		...
		if ( !is_wp_error($result) && isset($result['response']['code']) && ($result['response']['code'] == 200) && !empty($result['body']) ){
			$pluginInfo = Divi_Booster_PluginInfo::fromJson($result['body'], $this->debugMode);
		} else if ( $this->debugMode ) {
			$message = sprintf("The URL does not point to a valid plugin metadata file.", esc_url($url));
			if ( is_wp_error($result) ) {
				$message .= "HTTP error - " . $result->get_error_message();
			} else if ( isset($result['response']['code']) ) {
				$message .= "HTTP response code is " . $result['response']['code'] . " (expected: 200)";
			} else {
				$message .= "wp_remote_get() returned an unexpected result.";
			}
			$this->log_error($message, E_USER_WARNING);
		}
	...
	public function getInstalledVersion(){
		...
		if ( !is_file($this->pluginAbsolutePath) ) {
			...
			if ( $this->debugMode ) {
				$this->log_error(
					sprintf(
						"Can't to read the Version header - the file does not exist.",
						$this->pluginFile
					),
					E_USER_WARNING
				);
	...
	public function checkForUpdates(){
		...
		if ( $installedVersion === null ) {
			if ( $this->debugMode ) {
				$this->log_error(
					sprintf('Unable to determine installed version.', $this->pluginFile),
					E_USER_WARNING
				);
			}
			return null;
	...
	public function getUpdateState() {
		...
		if ( !empty($state) && isset($state->update) && is_object($state->update) ){
			$state->update = Divi_Booster_PluginUpdate::fromObject($state->update);
		}
		return $state;
	}
	...
	public function getUpdate() {
		...
		if ( !empty($state) && isset($state->update) && !empty($state->update) ){
			...
			if (($installedVersion !== null) && version_compare($update->version, $installedVersion, '>')){
				...
				if ($this->debugMode) {	$this->log_error("Can't determine installed version", E_USER_WARNING); }
			} else {
				return null; // no update
			}
		} else {
			// Couldn't retrieve update state (cache not valid?)
		}
		return null;
	}
	...
	public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
		...
		if ( $isRelevant && current_user_can('update_plugins') ) {
			...
			if ( !empty($linkText) ) {
				$pluginMeta[] = sprintf('<a href="%s">%s</a>', esc_attr(esc_url($linkUrl)), $linkText);
			}
		}
		return $pluginMeta;
	}
	...
	public function handleManualCheck() {
		...
		if ( $shouldCheck ) {
			...
			wp_redirect(esc_url_raw(add_query_arg(
					array(
					     'puc_update_check_result' => urlencode($status),
					     'puc_slug' => urlencode($this->slug),
					),
					is_network_admin() ? network_admin_url('plugins.php') : admin_url('plugins.php')
			)));
	...
	public function displayManualCheckResult() {
		if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
			...
			$last_error = get_option('puc_last_error');
			if ($last_error) { 
				printf('<div class="error"><p>%s</p></div>', htmlentities($last_error));
				update_option('puc_last_error', '');
				return;
			}
...
if ( !class_exists('Divi_Booster_PluginInfo') ):
...
if ( !class_exists('Divi_Booster_PluginUpdate') ):
 ...
 * @access public
 */
class Divi_Booster_PluginUpdate {
	public $id = 0;
	public $slug;
	public $version;
	public $homepage;
	...
	public static function fromJson($json, $triggerErrors = false){
		...
		$pluginInfo = Divi_Booster_PluginInfo::fromJson($json, $triggerErrors);
		if ( $pluginInfo != null ) {
