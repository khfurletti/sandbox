	...
	<head>
		...
		<title>
			...
			/* translators: %s: Blog name */
				esc_html__( '%s — Coming Soon', 'mojo-marketplace-wp-plugin' ),
			...
			<div class="content">
				...
				<?php if ( class_exists( 'Jetpack' ) && Jetpack::is_module_active( 'subscriptions' ) ) { ?>
					...
					<div class="bh_subscription_widget">
						...
						<div
							id="success"><?php esc_html_e( 'Thank you, please check your email to confirm your subscription.', 'mojo-marketplace-wp-plugin' ); ?></div>
						<div
							id="error"><?php esc_html_e( 'There was an error with your submission and you were not subscribed. Please try again with a different email address.', 'mojo-marketplace-wp-plugin' ); ?></div>
						<form action="" method="post" accept-charset="utf-8" id="subscribe-bh">
							...
							<label id="bh-subscribe-label" for="subscribe-field-bh">
								<?php esc_html_e( 'Email', 'mojo-marketplace-wp-plugin' ); ?>
							</label>
							<input
								type="email"
								name="email"
								...
								placeholder="<?php esc_attr_e( 'Enter your email address', 'mojo-marketplace-wp-plugin' ); ?>"
							>
						</span>
					...
					$.ajax({
						...
						success: function (response) {
							...
							if (status == 'success') {
								...
								$('#error').show().text('<?php esc_html_e( 'Your email address is already subscribed to this website. Stay tuned to your inbox for our updates or try a different email address.', 'mojo-marketplace-wp-plugin' ); ?>');

							} else if (status == 'invalid_email') {

								$('#error').show().text('<?php esc_html_e( 'There was an error with your submission and you were not subscribed. Please try again with a valid email address.', 'mojo-marketplace-wp-plugin' ); ?>');
